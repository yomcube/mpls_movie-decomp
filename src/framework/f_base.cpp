...
int fBase_c::executePack() {
    int res = commonPack(&fBase_c::execute, &fBase_c::preExecute, &fBase_c::postExecute);
#ifndef NSMBW
    while (m_nextBase != nullptr) {
        fBase_c *next = m_nextBase;
        m_nextBase = nullptr;
        next->commonPack(&fBase_c::execute, &fBase_c::preExecute, &fBase_c::postExecute);
    }
#endif
    return res;
}
...
#ifndef NSMBW
    if (connectParent->mDeleteRequested || connectParent->mLifecycleState == DELETING) {
        return nullptr;
    }
#endif
